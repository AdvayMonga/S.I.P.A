...
| Concurrent chats | built (M18) · M19 building | [concurrent-chats.md](design/concurrent-chats.md) | The switchboard: daemon's one serial conversation → a flat pool of up to 5 concurrent chat threads. One focused (chat module), rest in the panel as status boxes; swap to switch; Stop/Resolve per thread. M19 (fluid threads): thread-tagged push delivery, mid-flight → background hand-off, and Merge-back. |
