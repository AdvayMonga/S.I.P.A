...
| Research flow | built (v1, playbook) | [research.md](design/research.md) | No new server/tool — a SYSTEM playbook over existing web + vault tools + the agent loop. Shallow (inline) vs deep (decompose → iterate → fetch-before-cite → linked `Research/` note). Clarify-when-vague. Adversarial verify + code-side cite validation deferred. |
