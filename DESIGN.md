...
| Research flow | built | [research.md](design/research.md) | SYSTEM playbook over existing web + vault tools + the agent loop. Shallow (inline) vs deep (decompose → iterate → fetch-before-cite → **adversarially verify** → linked `Research/` note). Clarify-when-vague. `verify_claims` (`src/bot/verify.py`) fans out independent skeptic sub-agents to refute each finding. Code-side cite validation still deferred. |
...
| Context assembly v2 | built | [context-assembly.md](design/context-assembly.md) | Per-turn pushed retrieval: auto-inject profile + top-k memory + top-k vault (provenance-tagged, one char budget) into the system prompt. Retrieval goes agentic → automatic. Context-dependent follow-ups are rewritten to a standalone query before searching (`query.py`, gated). |
...
| Concurrent chats | built (M18 + M19) | [concurrent-chats.md](design/concurrent-chats.md) | The switchboard: daemon's one serial conversation → a flat pool of up to 5 concurrent chat threads. One focused (chat module), rest in the panel as status boxes; swap to switch; Stop/Resolve per thread. M19 (fluid threads): thread-tagged push delivery, mid-flight → background hand-off (no restart), and Merge-back. Backgrounding is user-driven; `delegate` fan-out stays. |
